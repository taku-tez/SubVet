...
export const cloudFingerprints: ServiceFingerprint[] = [
  {
    service: 'AWS S3',
    description: 'Amazon S3 bucket',
    cnames: ['*.s3.amazonaws.com', '*.s3-*.amazonaws.com', '*.s3.*.amazonaws.com'],
    fingerprints: [
      { type: 'http_body', pattern: 'NoSuchBucket', weight: 10, required: true },
      { type: 'http_body', pattern: 'The specified bucket does not exist', weight: 10 },
      { type: 'http_status', value: 404, weight: 2 }
    ],
    negativePatterns: [
      { type: 'http_body', pattern: 'AccessDenied', description: 'Bucket exists but access denied' },
      { type: 'http_status', value: 403, description: 'Access denied (bucket exists)' }
    ],
  ...
  },
  {
    ...
    fingerprints: [
      { type: 'http_body', pattern: 'Bad Request', weight: 4 },
      { type: 'http_body', pattern: "The request could not be satisfied", weight: 8 },
      { type: 'http_body', pattern: 'ERROR: The request could not be satisfied', weight: 9 },
      { type: 'http_header', header: 'x-cache', pattern: 'Error from cloudfront', weight: 10, required: true },
      { type: 'http_body', pattern: 'CloudFront', weight: 5 },
      { type: 'http_status', value: 403, weight: 2 }
    ],
    negativePatterns: [
      { type: 'http_header', header: 'x-cache', pattern: 'Hit from cloudfront', description: 'CDN cache hit' },
      { type: 'http_status', value: 200, description: 'Content serving' }
    ],
    minConfidence: 5,
    takeoverPossible: true,
  ...
  },
  {
    ...
    cnames: ['*.amplifyapp.com'],
    fingerprints: [
      { type: 'http_body', pattern: 'This page could not be found', weight: 8 },
      { type: 'dns_nxdomain', weight: 10 }
    ],
    takeoverPossible: true,
  ...
  },
  {
    ...
    fingerprints: [
      ...
      { type: 'http_body', pattern: 'The resource you are looking for has been removed', weight: 8 },
  ...
  },
  {
    ...
    cnames: ['*.run.app', 'ghs.googlehosted.com'],
    fingerprints: [
      { type: 'http_status', value: 404, weight: 3 },
